...
resource "azurerm_linux_virtual_machine" "kafkaServer" {
  ...
  provisioner "local-exec" {
    command = "ansible-playbook -i '${azurerm_public_ip.kafkaPublicIP.ip_address},' -u ${var.vmUserName} --private-key ${var.sshPrvtKeyPath} ../ansible/KafkaServer/apache_kafka.yml -e @../ansible/KafkaServer/stdds.json -e @../ansible/KafkaServer/tfms.json -e @../ansible/KafkaServer/tbfm.json"
  }
